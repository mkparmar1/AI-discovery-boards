...
import { Search, Star, ExternalLink, Cloud } from 'lucide-react'

const CloudAIToolsPage = () => {
  const [searchTerm, setSearchTerm] = useState('')
  const [sortBy, setSortBy] = useState<'name' | 'rating' | 'reviews'>('rating')

  const cloudTools = aiTools.filter(tool => 
    tool.category === 'Cloud AI Services' || 
    tool.tags.some(tag => tag.toLowerCase().includes('cloud')) ||
    tool.tags.some(tag => tag.toLowerCase().includes('api')) ||
    tool.tags.some(tag => tag.toLowerCase().includes('service'))
  )
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-4xl font-bold">Cloud AI Services</h1>
        </div>
        <p className="text-lg text-muted-foreground mb-6">
          Cloud-based AI services and APIs for scalable artificial intelligence solutions
        </p>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Search cloud AI services..."
              value={searchTerm}
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Found {sortedTools.length} cloud AI services
        </p>
      </div>
      ...
      {sortedTools.length === 0 && (
        <div className="text-center py-12">
          <p className="text-lg text-muted-foreground">
            No cloud AI services found matching your search.
          </p>
        </div>
      )}
    </div>
...
export default CloudAIToolsPage