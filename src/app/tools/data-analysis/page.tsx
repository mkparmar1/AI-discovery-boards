...
import { Search, Star, ExternalLink, Database } from 'lucide-react'

const DataAnalysisToolsPage = () => {
  const [searchTerm, setSearchTerm] = useState('')
  const [sortBy, setSortBy] = useState<'name' | 'rating' | 'reviews'>('rating')

  const dataTools = aiTools.filter(tool => 
    tool.category === 'Data Analysis' || 
    tool.tags.some(tag => tag.toLowerCase().includes('data')) ||
    tool.tags.some(tag => tag.toLowerCase().includes('analytics')) ||
    tool.tags.some(tag => tag.toLowerCase().includes('analysis'))
  )
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-4xl font-bold">Data Analysis Tools</h1>
        </div>
        <p className="text-lg text-muted-foreground mb-6">
          Powerful tools for data processing, analysis, and insights generation
        </p>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Search data analysis tools..."
              value={searchTerm}
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Found {sortedTools.length} data analysis tools
        </p>
      </div>
      ...
      {sortedTools.length === 0 && (
        <div className="text-center py-12">
          <p className="text-lg text-muted-foreground">
            No data analysis tools found matching your search.
          </p>
        </div>
      )}
    </div>
...
export default DataAnalysisToolsPage