...
const AutomationToolsPage = () => {
  ...
  const automationTools = aiTools.filter(tool => 
    tool.category === 'Automation' || 
    tool.tags.some(tag => tag.toLowerCase().includes('automation')) ||
    tool.tags.some(tag => tag.toLowerCase().includes('workflow')) ||
    tool.tags.some(tag => tag.toLowerCase().includes('bot'))
  )
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-lg text-muted-foreground mb-6">
          AI-powered automation tools to streamline workflows and boost productivity
        </p>
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Found {sortedTools.length} automation tools
        </p>
      </div>
      ...
      {sortedTools.length === 0 && (
        <div className="text-center py-12">
          <p className="text-lg text-muted-foreground">
            No automation tools found matching your search.
          </p>
        </div>
      )}
    </div>
